...
int main(int argc, char **argv)
{
    ...
    dynamic_reconfigure::Server<sensors::WH_DVLConfig> gain_srv;
    dynamic_reconfigure::Server<sensors::WH_DVLConfig>::CallbackType f;
    ...
    while (n.ok()) {
        ...
        if (rdi_dvl->fd > 0) {
            ...
            if (rdi_dvl->cmdMode) {
                rdi_dvl->getRawData();
                string printString = rdi_dvl->getPrintString();
                if (printString.compare("") != 0) {
                    //cout << "WH::DVL is recording data" << endl;
                    //myfile << printString;
                    //cout << printString;
                    ROS_INFO("DVL: %s", printString.c_str());
                }
            }
            else {
                rdi_dvl->getData();
