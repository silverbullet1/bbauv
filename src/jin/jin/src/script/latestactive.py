...
def sendMovement(f=0.0, h=None, sm=0.0, d=0.4):
    d = d if d else depth
...
def triangulate(doa1, doa2, R12):
    ...
    with open('result.txt', 'a') as writer:
        writer.write("DOA1: " + str(doa1) + "\t\t" + "DOA2: " + str(doa2) + "\t\t" + "Forward: " + str(forward) + "\n" + "Distance is: " + str(R2))
    return [doa2, abs(R2)]
