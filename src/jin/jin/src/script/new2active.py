...
def withinRange(angle):
    return 0 <= angle <= 20 or 340 <= angle <=360
...
def neotriangulate(doa1, doa2, R12):
    global angConst
    outer_ang = 360 - doa2
    C = outer_ang - angConst
    R2 = R12*((math.sin(np.deg2rad(angConst)))/(math.sin(np.deg2rad(C))))
    return [doa2, abs(R2)]
...
if __name__ == "__main__":
    ...
    while len(angFromPing) is not 2:
        ...
        while not withinRange(DOA):
            sendMovement(h=(DOA + heading['yaw'])%360)
            stopRobot()
            conn.close()
            (conn, addr) = s.accept()
            data = conn.recv(BUFFER_SIZE)
        ...
        if len(angFromPing) is not 2:
            stopRobot()
            sendMovement(h=((heading['yaw'] + angConst)%360), d=depth)
