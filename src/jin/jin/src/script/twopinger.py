...
def splitMsg(data):
    ...
    hydro4_complex = np.complex(float(real4), float(imag4))
    hydro5_complex = np.complex(float(real5), float(imag5))
    hydro6_complex = np.complex(float(real6), float(imag6))
    hydro7_complex = np.complex(float(real7), float(imag7))
    
    ping1 = [hydro0_complex,hydro1_complex, hydro2_complex, hydro3_complex]
    ping2 = [hydro4_complex,hydro5_complex, hydro6_complex, hydro7_complex]
    return [ping1, ping2]

def music_algo(ls, doa):
    global A, DOA, DOA2
    r_conv = 0
...
if __name__ == "__main__":
    ...
    angFromPing2.append(DOA2)
    print DOA, DOA2
    result1 = triangulate(angFromPing[0], angFromPing[1], forward)
    result2 = triangulate(angFromPing2[0], angFromPing2[1], forward)
