...
class Point():
    ...
    def __init__(self, x, y, angle, angle2):
        self.pos['x'] = x
        self.pos['y'] = y
    ...
    while len(angFromPing) is not 2:
        ...
        while len(ls) is not int(take):
            ...
            if data:
                ...
                data = conn.recv(BUFFER_SIZE)
        music_algo(ls)
        head = (DEGREE_TWO_PI+heading['yaw']+DOA)%(DEGREE_TWO_PI)
        if withinRange(DOA):
            angFromPing.append(Point(abs_pos['x'], abs_pos['y'], head, head))
        else:
            getBetterAngle(DOA)
        sendMovement(sm = 4, d=depth)
        ls = []
        print "Relative is: ", DOA, " degrees"
        print "Absolute is: ", head, " degrees"
