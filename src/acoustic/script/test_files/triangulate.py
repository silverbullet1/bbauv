...
def triangulate(doa1, doa2):
    ...
    heading1 = (h1+doa1) % (DEGREE_TWO_PI)
    heading2 = (h2+doa2) % (DEGREE_TWO_PI)
    if (((math.sin(heading1/DEGREE_PI*math.pi)) != 0) and ((math.sin(heading2/DEGREE_PI*math.pi)) != 0)):	
        m1 = math.tan((math.sin(heading1/DEGREE_PI*math.pi)),(math.cos(heading1/DEGREE_PI*math.pi))) 
        c1 = y1_pos - (m1*x1_pos)
        m2 =  math.tan((math.sin(heading2/DEGREE_PI*math.pi)) / (math.cos(heading2/DEGREE_PI*math.pi)))
        c2 = y2_pos - (m2*x2_pos)
