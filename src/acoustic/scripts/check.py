...
navClient = rospy.ServiceProxy("/navigation2D", navigation2D, self.navCallback())
...
abs_pos_sub = rospy.Subscriber("earth_odom", Odometry, self.DVL_callback)
depth_sub = rospy.Subscriber("/depth", depth, self.depth_callback)
compass_sub = rospy.Subscriber('/euler', compass_data, self.compass_callback)
...
def DVL_callback(self, data):
    global abs_pos
    abs_pos['x'] = data.pose.pose.x
    abs_pos['y'] = data.pose.pose.y
