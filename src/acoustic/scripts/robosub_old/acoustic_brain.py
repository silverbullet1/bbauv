#!/usr/bin/env python 
'''
Added dynHandler() as callback for dynServer
'''
import rospy	
import roslib
import time
...
from acoustic.cfg import acousticConfig
import actionlib
import numpy as np
from numpy import *
...
class AcousticNode(object):
    ...
    def __init__(self, param):
        ...
        self.dynServer = Server(acoustic, self.dynHandler)
    ...
    def dynHandler(self, config, level):
    	    self.fh = config['freq1']
    	    self.fl = config['freq2']
    	    self.pingerDistance = config['distance']
    	    self.elevationBound = config['elevation_angle']
    	    self.isLogged = config['isLog']
