...
class Comm:
    ...
    #Init subs
    def adjustStep(self):
        if(65 < self.elevation < 80):
            self.step_size = 2.0
        elif(40 < self.elevation < 55):
            self.step_size = 1.0
    ...
    def sendMovement(self,forward=0.0, sidemove=0.0, turn=None, depth=None, absolute=False, wait=True):
        ...
        if wait:
            ...
            self.motionClient.wait_for_result(timeout=rospy.Duration(0.2))


    def listen(self):
        self.stream_thread = threading.Thread(target=self.streamer.stream)
    ...
    def unregisterCurr(self):
        if self.compass_sub is not None:
