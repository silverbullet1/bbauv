...
rospy.init_node("acoustic_twoping")
...
try:
    ...
    rospy.loginfo("Ended waiting for controller")
...
while heading is None:
    rospy.loginfo("awiting for compass")
    rospy.sleep(rospy.Duration(0.5))
    rospy.loginfo("ended waiting for compass")
'''
...
def sendMovement(forward=0.0, turn=None, depth=1.0):
    global heading, locomotionClient
    if turn is None:
        print "turn was none"
        turn = heading
        print "turn now " + str(turn)
    else:
        turn += heading
    if forward is None:
        print "forward was none"
    if depth  is None:
        print "depth is None"
    rospy.loginfo("Fwd: %f, Turn: %f, Depth: %f" % (forward, turn, depth))
    #print turn
    ...
    rospy.loginfo("final turning angle is %f, normalized= %f" % (turn,
                                                                 normalized_angle(turn)))
    goal = ControllerGoal(forward_setpoint=0.0,
                          heading_setpoint=normalized_angle(turn),
                          sidemove_setpoint=0.0, depth_setpoint=depth)
    locomotionClient.send_goal_and_wait(goal)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if counter > 1 and overShotPinger(DOA):
            ...
            distanceToMove = distanceToPinger("MUSIC", 2)
            sendMovement(forward=0, depth=1, turn=DOA)
            print("Turning to: " + str(DOA))
            sendMovement(forward=1.0)
            print("Moved forward")
            sendMovement(depth=0)
